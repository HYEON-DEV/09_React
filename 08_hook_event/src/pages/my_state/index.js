...
const MyState = memo( () => {
     ...
     * - 화면에 출력할 용도의 변수 개념
     * - const [변수이름, 변수에 대한 setter 함수] = React.useState(변수의기본값);
    ...
    return(
        <MyStateContainer>
            ...
            <div>
                ...
                {/* 1) 상태변수 myName이 value 속성에 연결된 입력 요소 */}
                {/* 2) 이벤트 연결은 'on이벤트이름 (대소문자 주의), 바드시 함수와 연결해야 한다 */}
                {/* 3) value 속성은 반드시 상태변수만 연결 가능하고, value 속성을 사용할 경우 onChange 이벤트를 통해 상태변수에 입력값을 갱신해 줘야 한다 */}
                {/* 4) 기존 HTML 에서의 value 속성은 defaulValue속성을 사용해야 한다 */}
